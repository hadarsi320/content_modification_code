...
if __name__ == '__main__':
    ...
    parser.add_option('--mode', options=['single', 'multiple'], default='single')
    ...
    create_index(comp_trectext_file, comp_index, indri_path)
    generate_document_tfidf_files(qid, 1, competitor_list, sentence_workingset_file, output_dir=doc_tfidf_dir,
    ...
    for epoch in range(1, total_rounds + 1):
        ...
        doc_texts = load_trectext_file(comp_trectext_file)
        create_index(comp_trectext_file, comp_index, indri_path)
        generate_document_tfidf_files(qid, epoch + 1, competitor_list, sentence_workingset_file,
